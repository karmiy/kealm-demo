...
import VcTabs, { TabPane } from '../vc-tabs/src/public_api';
...
export default {
  ...
  props: {
    prefixCls: PropTypes.string.def('salus-tabs'),
    ...
    if (type === 'editable-card') {
      ...
      children.forEach((child, index) => {
        ...
        const closeIcon = closable ? (
          <i class={`salus-icon-pop-close-x`}
            ...
            props: {
              tab: (
                <div class={closable ? undefined : `${prefixCls}-tab-unclosable`}>
                  {getComponentFromProp(child, 'title')}
                  {closeIcon}
                </div>
              ),
            },
      ...
      if (!hideAdd) {
        tabBarExtraContent = (
          <span>
            <i class={`salus-icon-plus-o`} onClick={this.createNewTab} />
            {tabBarExtraContent}
          </span>
        );
      }
