...
import intersperse from '../_util/intersperse';
...
export default {
  ...
  props: initDefaultProps(FormItemProps, {
    hasFeedback: false,
    prefixCls: 'salus-form',
    colon: true,
    explainOutside: false,
    iconOutside: false,
  }),
  inject: {
    FormProps: { default: () => ({}) },
  ...
  methods: {
    ...
    renderHelp() {
      ...
      const children = help ? (
        <span class={`${prefixCls}-explain`} key="help">
          <transition {...transitionProps} key="help">
            <div class={{'salus-form-explain-outside': this.explainOutside}}>
              {help}
            </div>
          </transition>
        </span>
      ...
      if (children) {
        ...
        // 有explain，给form-item加类名
        this.$el && (this.helpShow ? this.$el.classList.add(`${prefixCls}-item-with-help`) : this.$el.classList.remove(`${prefixCls}-item-with-help`));
      }
    ...
    renderExtra() {
      ...
      const extraContents = extra ? extra.map(item => <div class={`${prefixCls}-extra`}>{item}</div>): null;
    ...
    renderValidateWrapper(c1, c2, c3) {
      ...
      if (validateStatus) {
        classes = classNames(`${props.prefixCls}-item-control`, {
          ...
          'icon-Outside' : props.iconOutside,
        });
      }
      let iconType = '';
