...
import Affix from '../affix/public_api';
...
export default {
    ...
    updateInk() {
      ...
      if (linkNode) {
        if(!this.$props.horizontal) {
          ...
          this.$refs.linkNode.style.left = `${linkNode.offsetLeft}px`;
          this.$refs.linkNode.style.width = `${linkNode.clientWidth}px`;
        }
      }
    },
  },
  ...
  render() {
    const { prefixCls, offsetTop, affix, showInkInFixed, activeLink, $slots, getContainer, horizontal } = this;

    // 圆点/下划线的样式
    const lineInkClass = classNames({
      [`${prefixCls}-ink`]: !horizontal,
      [`${prefixCls}-horizontal-line`]: horizontal,
    })
    const inkClass = classNames({
      visible: activeLink,
      [`${prefixCls}-ink-ball`]: !horizontal,
      [`${prefixCls}-horizontal-line-bar`]: horizontal,
    });
