import VcTable from '../vc-table/public_api';
import classNames from 'classnames';
import shallowEqual from 'shallowequal';
import Pagination from '../pagination/public_api';
import Spin from '../spin/public_api';
        ...
        if ((column.filters && column.filters.length > 0) || column.filterDropdown) {
          ...
          filterDropdown = (
            <FilterDropdown
              ...
              dropdownPrefixCls={dropdownPrefixCls || 'salus-dropdown'}
              getPopupContainer={this.getPopupContainer}
              key="filter-dropdown"
            />
          );
        ...
        if (column.sorter) {
          ...
          sortButton = (
            <div class={`${prefixCls}-column-sorter`} key="sorter">
              <span class={`${prefixCls}-column-sorter-up ${isAscend ? 'on' : 'off'}`}>
                <i class="salusicon salus-icon-triangle-up" />
              </span>
              <span class={`${prefixCls}-column-sorter-down ${isDescend ? 'on' : 'off'}`}>
                <i class="salusicon salus-icon-triangle-down"/>
              </span>
            </div>
          );
