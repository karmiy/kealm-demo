...
import { Tree as VcTree, TreeNode } from '../vc-tree/public_api';
...
export default {
  ...
  methods: {
    renderSwitcherIcon({ isLeaf, expanded, loading }) {
      ...
      if (loading) {
        return <i class="salus-icon-loading"></i>
      }
      if (showLine) {
        if (isLeaf) {
          //return <i class="salus-icon-doc-surpport-o"></i>
          return <i class={`salus-icon-triangle-${expanded ? 'down' : 'right'}`}></i>
        }
        /*return (
          <Icon
        ...
        // return <i class="salus-icon-folder-o"></i>
        return <i class={`salus-icon-triangle-${expanded ? 'down' : 'right'}`}></i>
      } else {
        if (isLeaf) {
          return null;
