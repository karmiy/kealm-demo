...
import { Item as MenuItem, ItemGroup as MenuItemGroup } from '../vc-menu/public_api';
...
const Select = {
  ...
  methods: {
    ...
    onMenuSelect({ item }) {
      ...
      const props = this.$props;
      // 如果是多选，当前选中数量已经等于最多选择个数，return
      if (isMultipleOrTags(props) && props.maxMultipleCount && props.maxMultipleCount <= value.length)
        return;
    ...
    _getInputElement() {
      ...
      return (
        <div class={`${props.prefixCls}-search-field-wrap`} onClick={this.inputClick}>
          ...
          <span
            ...
            // ref='inputMirrorRef'
            class={`${props.prefixCls}-search-field-mirror`}
          >
            {inputValue}&nbsp;
          </span>
        </div>
    ...
    renderFilterOptions() {
      ...
      if (tags) {
        ...
        if (inputValue) {
          ...
          if (notFindInputItem) {
            const p = {
              // attrs: UNSELECTABLE_ATTRIBUTE, 灵异事件，加上去li上属性就炸了
              key: inputValue,
              props: {
                value: inputValue,
                role: 'option',
      ...
      if (!options.length && notFoundContent) {
        const p = {
          // attrs: UNSELECTABLE_ATTRIBUTE, 灵异事件，加上去li上属性全炸了
          key: 'NOT_FOUND',
          props: {
            value: 'NOT_FOUND',
            disabled: true,
    ...
    renderFilterOptionsFromChildren(children = [], childrenKeys, menuItems) {
      ...
      children.forEach(child => {
        ...
        if (this._filterOption(inputValue, child)) {
          const p = {
            attrs: {
              // ...UNSELECTABLE_ATTRIBUTE, 灵异事件，加上去li上属性全炸了
              ...getAttrs(child),
            },
            key: childValue,
            props: {
    ...
    renderTopControlNode() {
      ...
      if (isSingleMode(props)) {
        ...
        } else if(!isMultipleOrTags(props)) { // 单选才有这个
          innerNode = [
            selectedValue,
            <div
        ...
        if (maxTagCount !== undefined && value.length > maxTagCount) {
          ...
          /*maxTagPlaceholderEl = (
            <li
              ...
              class={`${prefixCls}-selection-choice ${prefixCls}-selection-choice-disabled`}
              key="maxTagPlaceholder"
              title={toTitle(content)}
            >
              <div class={`${prefixCls}-selection-choice-content`}>{content}</div>
            </li>
          );*/
          // 有最大显示个数，剩余直接li里...就行了
          maxTagPlaceholderEl = (
            <li
              ...
              role="presentation"
              onMousedown={preventDefaultEvent}
              class={`${prefixCls}-selection-ellipsis`}
              key="maxTagPlaceholder"
              title={toTitle(content)}
            >
              {content}
        ...
        if (isMultipleOrTags(props)) {
          selectedValueNodes = limitedCountValue.map(singleValue => {
            ...
            return (
              <li
                style={{...UNSELECTABLE_STYLE, ...tagWidth && {width: `${tagWidth}px`}}}
                {...{ attrs: UNSELECTABLE_ATTRIBUTE }}
                onMousedown={preventDefaultEvent}
              ...
              >
                ...
                {disabled ? null : (
                  <span
                    ...
                    class={`${prefixCls}-selection-choice-remove`}
                  >
                    {removeIcon || <i class={`${prefixCls}-selection-choice-remove-icon`}>×</i>}
                  </span>
                )}
              </li>
            );
    ...
    renderClear() {
      ...
      const clear = (
        <span
          key="clear"
          class={`${prefixCls}-selection-clear`}
          onMousedown={preventDefaultEvent}
          style={UNSELECTABLE_STYLE}
          {...{ attrs: UNSELECTABLE_ATTRIBUTE }}
          onClick={this.onClearSelection}
        >
          {clearIcon || <i class={`${prefixCls}-selection-clear-icon`}>×</i>}
        </span>
      );
      // 多选都不要清除图标，都让弹出框的重选来操作
      if (!allowClear || isMultipleOrTags(this.$props)) {
        return null;
      }
      if (isCombobox(this.$props)) {
        if (inputValue) {
    ...
    renderExtraHeader() {
      const props = this.$props;
      const { prefixCls, allowClear, showSearch, showSelectAll, showSelectInverse } = props;
      let searchWrap = null,
          selectAllWrap = null;
      if(isMultipleOrTags(props)) {
        if(showSearch) {
          ...
          searchWrap = (
            <div class={`${prefixCls}-search-container`}>
              <div className={`${prefixCls}-search-field-wrap`}>
                <InputSearch {...searchProps} ref={'InputSearch'} />
              </div>
            </div>
          )
        }
        if(allowClear || showSelectAll || showSelectInverse) {
          selectAllWrap = (
            <div class={`${prefixCls}-selectAll-wrap`}>
              {showSelectAll && <div class={`${prefixCls}-select-btn`}>
                <a onClick={this.onAllSelection}>全选</a>
              </div>}
              {showSelectInverse && <div class={`${prefixCls}-select-btn`}>
                <a onClick={this.onInverseSelection}>反选</a>
              </div>}
              {allowClear && <div class={`${prefixCls}-select-btn`}>
                <a onClick={this.onClearSelection}>重选</a>
              </div>}
            </div>
          )
        }
      }
      return [searchWrap, selectAllWrap];
    },
    /**
     *  清空搜索框
     */
    clearSearchInput() {
      const search = this.$refs.InputSearch;
      if(search) {
        search.$children[0].stateValue = '';
      }
    },
    /**
     *  全选
     */
    onAllSelection() {
      let { _inputValue: inputValue } = this.$data;
      // 先过滤数据
      const filterOptions = inputValue ? this._options.filter(child => {
        return !!this._filterOption(inputValue, child);
      }) : this._options;
      // 全选
      this.fireChange(filterOptions.map(child => {
     ...
     */
    onInverseSelection() {
      let { _inputValue: inputValue = '', _value: value = [] } = this.$data;
      // 先过滤数据
      const filterOptions = (inputValue || value.length) ? this._options.filter(child => {
        const optionValue = getValuePropValue(child);
        return !!this._filterOption(inputValue, child) && !value.includes(optionValue);
      }) : this._options;
      // 反选
      this.fireChange(filterOptions.map(child => {
        return getValuePropValue(child);
  ...
  render() {
    ...
    const rootCls = {
      [prefixCls]: true,
      [`${prefixCls}-open`]: open,
      /*[`${prefixCls}-focused`]: open || !!this._focused,*/ // 不要这个focused样式
    ...
    // 渲染列表头
    const extraHeader = this.renderExtraHeader();
    return (
      <SelectTrigger
