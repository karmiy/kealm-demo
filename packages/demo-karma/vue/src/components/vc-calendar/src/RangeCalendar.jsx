...
const RangeCalendar = {
  props: {
    ...
    operationRegxInput: PropTypes.object,
  ...
  data() {
    ...
    return {
      ...
      dateRangeClickTimes: 0, // 选择日期点击次数（第一下格式化为00:00:00，第二下23:59:59，2次后重置）
    };
  },
  watch: {
    value() {
  ...
  methods: {
    ...
    /* 累加日期点击次数 */
    onRangeTimeClick() {
      const preTimes = this.dateRangeClickTimes;
      this.dateRangeClickTimes = (this.dateRangeClickTimes + 1) % 2;
      return preTimes;
    },
    onSelect(value) {
      const { type, sSelectedValue, prevSelectedValue, firstSelectedValue } = this;
      const { timePicker, showOperation } = this.$props;
      const isNeedAdjustTimes = !timePicker && !showOperation; // 是否需要矫正时分秒和之前选择的一致（时间选择不需要，纯日期需要）
        let nextSelectedValue;
      if (type === 'both') {
        if (!firstSelectedValue) {
          isNeedAdjustTimes && syncTime(prevSelectedValue[0], value);
          nextSelectedValue = [value];
        } else if (this.compare(firstSelectedValue, value) < 0) {
          isNeedAdjustTimes && syncTime(prevSelectedValue[1], value);
          nextSelectedValue = [firstSelectedValue, value];
        } else {
          isNeedAdjustTimes && syncTime(prevSelectedValue[0], value);
          isNeedAdjustTimes && syncTime(prevSelectedValue[1], firstSelectedValue);
    ...
    // 自定义日期输入框调用
    // 私有变量_inputCustomStartValue、_inputCustomEndValue，用于暂存自定义数据，防止先输入右侧在输入左侧时，传给props.onInputSelect的右侧是undefined
    onStartInputSelectCustom(value) {
      this._inputCustomStartValue = value;
      const endValue = this._inputCustomEndValue || this.sSelectedValue[1];
      // 初始日期不能比结束日期晚
      if(endValue && value.isAfter(endValue))
        return;
      // 检测是否日期不可用
      if(this.checkoutDisabled(value, 'start'))
        return false;
      //this.props.onInputSelect([value, endValue]);
      // console.log([value, endValue]);
      // 右侧输入框已经输入则一起触发onSelect
      if(endValue) {
        this.onSelect(value);
        this.onSelect(endValue);
      }
    },

    onEndInputSelectCustom(value) {
      this._inputCustomEndValue = value;
      const startValue = this._inputCustomStartValue || this.sSelectedValue[0];
      // 初始日期不能比结束日期晚
      if(startValue && value.isBefore(startValue))
        return;
      // 检测是否日期不可用
      if(this.checkoutDisabled(value, 'end'))
        return false;
      //this.props.onInputSelect([startValue, value]);
      // console.log([startValue, value]);
      // 右侧自定义时间需要两个输入框都有了才触发onSelect
      if(startValue) {
        this.onSelect(startValue);
        this.onSelect(value);
      }
    },

    // 校验自定义输入框的日期是否为不可用的，不可用return true，type为start/end
    checkoutDisabled(value, type) {
      if(!value)
        return true;
      const {
        disabledTime,
      ...
      // 校验时分秒
      if(disabledTime && disabledTime(value, type)) {
        const settingDisabledTime = disabledTime(value, type),
          { disabledHours, disabledMinutes, disabledSeconds } = settingDisabledTime;
        if( disabledHours().includes(parseInt(value.hour())) )
          return true;
        if( disabledMinutes().includes(parseInt(value.minute())) )
          return true;
        if( disabledSeconds().includes(parseInt(value.seconds())) )
          return true;
      }
  ...
  render() {
    ...
    const className = {
      ...
      [`${prefixCls}-time`]: props.showOperation || props.timePicker, // 让取消和确定按钮靠右排列
    };
    const baseProps = {
      props,
      on: $listeners,
    ...
    const newProps = {
      ...
      on: {
        rangeTimeClick: this.onRangeTimeClick,
    ...
    const leftPartProps = mergeProps(baseProps, newProps, {
      props: {
        ...
        clearIcon,
        onInputSelectCustom: this.onStartInputSelectCustom,
      },
      on: {
    ...
    const rightPartProps = mergeProps(baseProps, newProps, {
      props: {
        ...
        enableNext: true,
        clearIcon,
        onInputSelectCustom: this.onEndInputSelectCustom,
      },
      on: {
    ...
    let OkButtonNode = null, cancelButtonNode = null;
    if (showOkButton || props.showOperation) {
      ...
      const cancelButtonProps = mergeProps(baseProps, {
        props: {
        },
        on: {
      ...
      cancelButtonNode = <CancelButton key="cancelButtonNode" {...cancelButtonProps} />;
    }
    const buttonNodeGroup = (OkButtonNode && OkButtonNode) ? (
      <div>
        {cancelButtonNode}
        {OkButtonNode}
      </div>
    ...
    return (
      <div ref="rootInstance" class={className} tabIndex="0" onKeydown={this.onKeyDown}>
        ...
        <div class={`${prefixCls}-panel`}>
          ...
          >
            {(props.timePicker || props.showOperation) && <div class={`${prefixCls}-range-tips-left`}>自定义</div>}
            <CalendarPart {...leftPartProps} />
            {/*<span class={`${prefixCls}-range-middle`}>~</span>*/}
            {(props.timePicker || props.showOperation) && <div class={`${prefixCls}-range-tips-middle`}>至</div>}
            <CalendarPart {...rightPartProps} />
          </div>
          <div class={cls}>
            {showToday || props.timePicker || showOkButton || extraFooter || props.showOperation ? (
