...
const DateTBody = {
  ...
  methods: {
    ...
    toSelect(current) {
      ...
      const { select = noop, rangeTimeClick = noop } = $listeners;
      const times = rangeTimeClick();
      if(times === 0) {
        select(current.startOf('days'));
      }else if(times === 1){
        select(current.endOf('days'));
      }else {
        // 不是range-pick正常调用即可
        select(current);
      }
    ...
    for (iIndex = 0; iIndex < DateConstants.DATE_ROW_COUNT; iIndex++) {
      ...
      for (jIndex = 0; jIndex < DateConstants.DATE_COL_COUNT; jIndex++) {
        ...
        dateCells.push(
          <td
            key={passed}
            onClick={disabled ? noop : this.toSelect.bind(null, current)}
