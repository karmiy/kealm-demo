...
const Notification = {
  ...
  render(h) {
    const { prefixCls, notices, remove, getTransitionName, options } = this;
    ...
    const noticeNodes = notices.map((notice, index) => {
      ...
      return (
        <Notice {...noticeProps}></Notice>
      );
    });
    const className = {
      [`${prefixCls}-${options.pos}`]: 1,
    };
    ...
    return (
      ...
      >
        {options.animate ?
          <transition-group {...transitionProps} tag="div">{noticeNodes}</transition-group>
          :
          noticeNodes
        }
      </div>
...
Notification.newInstance = function newNotificationInstance(properties, callback) {
  const { getContainer, style, class: className, options, ...props } = properties || {};
  ...
  new Vue({
    ...
    render() {
      const p = {
        props: {
          ...props,
          options: Object.assign(options, {pos: this.pos}),
        },
        ref: 'notification',
        style,
        class: className,
