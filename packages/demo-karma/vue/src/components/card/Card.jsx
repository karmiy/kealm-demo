...
import Tabs from '../tabs/public_api';
import Row from '../row/public_api';
import Col from '../col/public_api';
...
export default {
  ...
  props: {
    ...
    defaultShadow: PropTypes.bool.def(false),
  ...
  methods: {
    ...
    isContainGrid(obj = []) {
      let containGrid;
      obj.forEach(element => {
        if (element && getSlotOptions(element).__SL_CARD_GRID) {
          containGrid = true;
        }
      });
      return containGrid;
  ...
  render() {
    ...
    const classString = {
      ...
      [`${prefixCls}-shadow`]: defaultShadow,
