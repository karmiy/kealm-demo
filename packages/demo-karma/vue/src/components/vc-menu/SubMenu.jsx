...
import getScrollContainerList, { isScrollContainer } from '../_util/getScrollContainer';
...
const SubMenu = {
  ...
  props: {
    ...
    active: PropTypes.bool, // ttodo: remove
    ...
    inlineIndent: PropTypes.number.def(10),
    ...
    iconRight: PropTypes.bool, // 新增iconRight
    inDropDown: PropTypes.bool.def(false), // 新增是否是下拉菜单属性，设为true一脸会有边框
    scrollbar: PropTypes.bool.def(false), // 新增滚动条菜单
    parentSubMenu: PropTypes.object, // 新增，该subMenu的上级subMenu
    hasArrow: PropTypes.bool.def(true), // 新增，是否显示菜单箭头
  },
  mixins: [BaseMixin],
  isSubMenu: true,
  data() {
  ...
  methods: {
    ...
    renderChildren(children) {
      ...
      const subPopupMenuProps = {
        props: {
          ...
          inDropDown: props.inDropDown, // inDropdown往下传
          scrollbar: props.scrollbar, // scrollbar往下传
          children,
          parentSubMenu: this, // 本层subMenu实例往下传，为了上子组件里有subMenu的找到它
        },
        on: {
    ...
    /**
     * 获取trigger层DOM，提供给子组件的subMenu的trigger的插入位置
     */
    getPopupDomNode() {
  ...
  render(h) {
    ...
    const titleProps = {
      ...
      class: {
        ...
        [`${prefixCls}-title-icon-right`]: !!props.iconRight,
      },
      ref: 'subMenuTitle',
    };
    ...
    const title = (
      <div {...titleProps}>
        {/* 有inDropdown时使用右侧指标 */}
        {props.hasArrow && (props.inDropDown ? <i class="salus-icon-arrow-right-o"></i> : <i class="salus-icon-triangle-down"></i>)}
    ...
    const getPopupContainer = this.parentMenu.isRootMenu
      ...
      : (this.parentSubMenu ? this.parentSubMenu.getPopupDomNode : triggerNode => triggerNode.parentNode);
    ...
    // 如果是垂直，给弹出框加宽140
    const popupStyle = props.mode === 'vertical' ? {
      minWidth: '140px'
    } :  {}
    // 当菜单栏ul有滚动条时，右偏移placements加个10px
    const _placements = JSON.parse(JSON.stringify(placements));
    if(props.scrollbar && this.$el && this.$el.parentNode && isScrollContainer(this.$el.parentNode)) {
      _placements['rightTop']['offset'][0] = 14;
    }
    return (
      <li {...liProps} role="menuitem">
        ...
        {!isInlineMode && (
          <Trigger
            ...
            builtinPlacements={_placements}
            builtinPlacements={Object.assign({}, _placements, props.builtinPlacements)}
