...
const SubPopupMenu = {
  ...
  props: initDefaultProps(
    {
      ...
      inDropDown: PropTypes.bool, // 新增是否是下拉菜单属性，设为true一脸会有边框
      scrollbar: PropTypes.bool, // 新增是否滚动条
      parentSubMenu: PropTypes.object, // 新增
    },
    {
      prefixCls: 'rc-menu',
      mode: 'vertical',
    ...
    renderCommonMenuItem(child, i, extraProps) {
      ...
      const newChildProps = {
        props: {
          ...
          inDropDown: props.inDropDown, // inDropDown也往下传
          scrollbar: props.scrollbar, // scrollbar也往下传
          parentSubMenu: props.parentSubMenu, // parentSubMenu往下传给子组件里的subMenu
          ...extraProps,
        },
        on: {
          click: e => {
  ...
  render() {
    ...
    // 使用select下拉选择器时，为了把传过来的props.prefixCls的salus-select-menu改成salus-select-option
    //const className = classNames(props.prefixCls, `${props.prefixCls}-${props.mode}`);
    const className = classNames(/-menu$/.test(props.prefixCls) ? props.prefixCls : `${props.prefixCls}-menu`, `${props.prefixCls}-${props.mode}`);
    const domWrapProps = {
      props: {
        tag: 'ul',
      ...
      class: classNames(className, {
        'salus-dropdown-scrollbar' : props.scrollbar,// 滚动条样式
      }),
