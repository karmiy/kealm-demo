...
function isDateArray(value) {
  if (Array.isArray(value)) {
    return (
      value.length === 0 || value.findIndex(val => val === undefined || (val instanceof Date)) !== -1
    );
  }
  return false;
}
...
export const RangePickerProps = () => ({
  ...PickerProps(),
  value: PropTypes.custom(isDateArray),
  defaultValue: PropTypes.custom(isDateArray),
  defaultPickerValue: PropTypes.custom(isDateArray),
  ...
  disabledTime: PropTypes.func,
  showToday: PropTypes.bool,
  rangeBtnType: PropTypes.oneOfType([PropTypes.array, PropTypes.string]),
