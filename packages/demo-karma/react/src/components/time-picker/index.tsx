...
import RcTimePicker from '../rc-time-picker/TimePicker';
...
class TimePicker extends React.Component<TimePickerProps, any> {
  ...
  renderInputIcon(prefixCls: string) {
    ...
    /*const clockIcon = (suffixIcon &&
      ...
      ))) || <a className={`${prefixCls}-panel-change-time`} title='change' />;*/
    const clockIcon = (suffixIcon &&
      (React.isValidElement<{ className?: string }>(suffixIcon) ? (
        React.cloneElement(suffixIcon, {
          className: classNames(suffixIcon.props.className, `${prefixCls}-clock-icon`),
  ...
  renderClearIcon(prefixCls: string) {
    ...
    const clearIcon = <a className={`${prefixCls}-panel-clear-btn`} title={this.props.clearText || 'clear'} />;
    ...
    <ConfigConsumer>
      {({ getPopupContainer: getContextPopupContainer, getPrefixCls }: ConfigConsumerProps) => {
        ...
        const pickerAddon = (panel: React.ReactElement<any>) =>
          addon ? <div className={`${prefixCls}-panel-addon`}><div className={`${prefixCls}-addon`}>{addon(panel)}</div></div> : null;
