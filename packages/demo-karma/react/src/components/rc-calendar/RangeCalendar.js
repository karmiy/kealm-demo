...
class RangeCalendar extends React.Component {
  ...
  onStartInputSelectCustom = (value) => {
    ...
    const endValue = this._inputCustomEndValue || this.state.selectedValue[1];
    // 初始日期不能比结束日期晚
    ...
    this.props.onSelect([value, endValue]);
  }

  onEndInputSelectCustom = (value) => {
    this._inputCustomEndValue = value;
    const startValue = this._inputCustomStartValue || this.state.selectedValue[0];
    // 初始日期不能比结束日期晚
    ...
    this.props.onSelect([startValue, value]);
  }
  ...
  checkoutDisabled = (value, type) => {
    ...
    // 校验时分秒
    if(disabledTime && disabledTime.name !== 'noop') {
    ...
    return (
      ...
      >
        ...
        <div className={`${prefixCls}-panel`}>
          ...
          >
            {(props.timePicker || props.showOperation) && <div className={`${prefixCls}-range-tips-left`}>自定义</div>}
            <CalendarPart
              {...props}
              {...newProps}
            ...
            {(props.timePicker || props.showOperation) && <div className={`${prefixCls}-range-tips-middle`}>至</div>}
            <CalendarPart
              {...props}
              {...newProps}
              hoverValue={hoverValue}
            ...
            {(showToday || props.timePicker || showOkButton || extraFooter || props.showOperation) ? (
              <div className={`${prefixCls}-footer-btn`}>
                ...
                {props.timePicker ?
                  ...
                  /> : null}
                {/* 显示确定取消按钮 */}
                {(showOkButton || props.showOperation) ?
                  <div>
                    <CancelButton
                      {...props}
