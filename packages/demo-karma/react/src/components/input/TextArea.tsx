...
class TextArea extends React.Component<TextAreaProps, TextAreaState> {
  ...
  renderTextArea = ({ getPrefixCls }: ConfigConsumerProps) => {
    const { prefixCls: customizePrefixCls, className, disabled, limitCount } = this.props;
    const { ...props } = this.props;
    const otherProps = omit(props, ['prefixCls', 'onPressEnter', 'autosize', 'limitCount']);
    ...
    // 如果有字数限制，要限制value 和 defaultValue
    if( limitCount && limitCount > 0 ) {
      otherProps.value && (otherProps.value = otherProps.value.substring(0, limitCount));
      otherProps.defaultValue && (otherProps.defaultValue = otherProps.defaultValue.substring(0, limitCount));
    }
    return (
      <textarea
        {...otherProps}
