...
const InputSizes = tuple('small', 'middle', 'large');
...
class Input extends React.Component<InputProps, any> {
  ...
  renderClearIcon() {
    ...
    return (
      <i className={'salus-icon-error-msg'} onClick={this.handleReset}></i>
    );
  }
  ...
  renderLabeledInput(prefixCls: string, children: React.ReactElement<any>) {
    ...
    const addonAfterNodeType = (addonAfter as React.ReactElement<any>)!.type === Button ? <span className={`${prefixCls}-suffix`}>{addonAfter}</span> : <span className={addonClassName}>{addonAfter}</span>;
    const addonAfterNode = addonAfter ? addonAfterNodeType : null;

    const mergedWrapperClassName = classNames(`${prefixCls}-wrapper`, {
      [wrapperClassName]: addonBefore || addonAfter,
      [`${prefixCls}-search`]: className && className.includes('enter-button'),
      [`${prefixCls}-search-enter-button`]: className && className.includes('enter-button'),
    });

    const mergedGroupClassName = classNames(className, `${prefixCls}-group-wrapper`, {
      [`${prefixCls}-group-wrapper-sm`]: size === 'small',
      [`${prefixCls}-group-wrapper-lg`]: size === 'large',
      [`${prefixCls}-affix-wrapper`]: className && className.includes('enter-button'),
    });
  ...
  renderInput(prefixCls: string) {
    const { className, addonBefore, addonAfter, limitCount } = this.props;
    const { value } = this.state;
    ...
    //
    let inputValue = fixControlledValue(value);
    // 如果有字数限制要截取
    limitCount && inputValue && (inputValue = inputValue.substring(0, limitCount));
