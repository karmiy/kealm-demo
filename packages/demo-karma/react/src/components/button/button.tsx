...
const ButtonTypes = tuple('primary', 'dashed', 'link', 'danger', 'success');
export type ButtonType = (typeof ButtonTypes)[number];
const ButtonShapes = tuple('circle');
export type ButtonShape = (typeof ButtonShapes)[number];
const ButtonSizes = tuple('middle', 'small');
...
class Button extends React.Component<ButtonProps, ButtonState> {
  ...
  static propTypes = {
    ...
    shap: PropTypes.oneOf(ButtonShapes),
  ...
  renderButton = ({ getPrefixCls, autoInsertSpaceInButton }: ConfigConsumerProps) => {
    ...
    const classes = classNames(prefixCls, className, {
      [`${prefixCls}-${type}`]: type,
      [`${prefixCls}-full`]: full,
      [`${prefixCls}-circle`]: shap,
      [`${prefixCls}-${sizeCls}`]: sizeCls,
      [`${prefixCls}-icon`]: !children && children !== 0 && icon,
      [`${prefixCls}-icon1`]: children && icon,
      [`${prefixCls}-loading`]: loading,
      [`${prefixCls}-clicked`]: this.state.clicked,
    ...
    const iconType = loading ? 'salus-icon-loading salus-icon-loading-white' : icon;
    /*const iconNode = iconType ? <Icon type={iconType} /> : null;*/
    const iconNode = iconType ? <i className={iconType}></i> : null;
    const kids =
      children || children === 0
