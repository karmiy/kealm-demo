...
export default (props: CardMetaProps) => (
  <ConfigConsumer>
    {({ getPrefixCls }: ConfigConsumerProps) => {
      ...
      const timeInfoDom = timeInfo ? <span className={`${prefixCls}-meta-time-info`}>{timeInfo}</span> : null;
      const metaExtraDom = metaExtra ? <div className={`${prefixCls}-extra`}>{metaExtra}</div> : null;
      const MetaDetail =
