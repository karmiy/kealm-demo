...
import Menu from '../rc-menu';
import { childrenToArray as toArray } from './util';
...
export default class DropdownMenu extends React.Component<Partial<IDropdownMenuProps>> {
  ...
  // 渲染控制按钮 全选/重选 等
  public renderControlButton = () => {
    if(!this.props.allowClear) {
      return null;
    }
    return (
      <div className={'salus-select-selectAll-wrap'}>
        <div className={'salus-select-select-btn'}>
          <a onClick={() => { this.props.fireChange && this.props.fireChange([]) }}>重选</a>
        </div>
      </div>
    )
  }
  ...
  public renderSearchInput = () => {
    /*return (
      <div className={'salus-select-search-container'}>
        <div className={'salus-select-search-field-wrap'}>
          <Input.Search
            placeholder="请输入搜索"
    ...
    if(this.props.showSearch && this.props.getInputElement){
  ...
  public render() {
    ...
    const renderControlButton = this.renderControlButton();
    const renderSearchInput = this.renderSearchInput();
    return renderMenu ? (
      <div
        style={{
          overflow: 'auto',
