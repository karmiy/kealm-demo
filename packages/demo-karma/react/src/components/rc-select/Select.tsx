...
class Select extends React.Component<Partial<ISelectProps>, ISelectState> {
  ...
  public static getOptionsInfoFromProps = (
    ...
    options.forEach(option => {
      ...
      (optionsInfo as any)[getMapKey(singleValue)] = {
        option,
        value: singleValue,
    ...
    if (preState) {
      ...
      if (value) {
        (value as string[]).forEach(v => {
          ...
          if (!(optionsInfo as any)[key] && oldOptionsInfo[key] !== undefined) {
            (optionsInfo as any)[key] = oldOptionsInfo[key];
          }
        });
      }
    }
  ...
  public getPlaceholderElement = () => {
    ...
    if (isCombobox(props) && value.length === 1 && (state.value && !(state as any).value[0])) {
      hidden = false;
    }
  ...
  public getInputElement = () => {
    ...
    return (
      <div className={`${props.prefixCls}-search-container`}>
        <div className={`${props.prefixCls}-search-field-wrap`}>
          ...
          <span ref={this.saveInputMirrorRef} className={`${props.prefixCls}-search-field-mirror`}>
  ...
  public renderTopControlNode = () => {
    ...
    if (isSingleMode(props)) {
      ...
      if (value.length) {
        ...
        //let opacity = 1; // 搜索框位置调整到DropdownMenu.tsx，不需要在这设置选则时的透明度
        if (!showSearch) {
      ...
      // 搜索框迁移至DropdownMenu.tsx，这里隐藏（不知为何去掉会导致点空白弹框不消失，有空再研究）
      selectedValueNodes.push(
        <li className={`${prefixCls}-search ${prefixCls}-search-inline`} key="__input" style={{maxWidth: 0,opacity: 0, position: 'relative', zIndex: -10}}>
          {this.getInputElement()}
        </li>,
      );
  ...
  public renderClear() {
    ...
    const clear = (
      ...
      >
        {clearIcon || <i className={`${prefixCls}-selection-clear-icon`}>×</i>}
      </span>
    );
    if (!allowClear) {
      return null;
  ...
  public render() {
    ...
    Object.keys(props).forEach(key => {
      if (
        ...
        (dataOrAriaAttributeProps as any)[key] = (props as any)[key];
      }
    });
