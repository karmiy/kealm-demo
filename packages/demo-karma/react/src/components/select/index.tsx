...
export default class Select<T = SelectValue> extends React.Component<SelectProps<T>, {}> {
  ...
  /*componentDidMount() {
    /!* 把rc-select里
      salus-select-selection__rendered类名改为salus-select-selection-rendered,
      salus-select-selection__clear类名改为salus-select-selection-clear
    *!/
    const node = ReactDom.findDOMNode(this) as HTMLElement,
           renderedNode = node && node.querySelector('.salus-select-selection__rendered'),
           clearNode = node && node.querySelector('.salus-select-selection__clear');
    renderedNode && (renderedNode.classList.remove('salus-select-selection__rendered'), renderedNode.classList.add('salus-select-selection-rendered'));
    clearNode && (clearNode.classList.remove('salus-select-selection__clear'), clearNode.classList.add('salus-select-selection-clear'));
  }*/
  focus() {
    this.rcSelect.focus();
  }
  ...
  renderSuffixIcon(prefixCls: string) {
    ...
    if (loading) {
      ...
      return <i className={'salus-icon-loading'} />;
    }
    //return <Icon type="down" className={`${prefixCls}-arrow-icon`} />;
    return <i className={'salus-icon-triangle-down'} />;
  }
  ...
  renderSelect = ({
    ...
    const finalRemoveIcon = (removeIcon &&
      (React.isValidElement<{ className?: string }>(removeIcon)
        ...
        : removeIcon)) || <i className={`salus-icon-pop-close-o`} />;
